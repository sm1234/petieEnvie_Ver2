...
function slideUp(){	
if((jQuery('#prev-panel').css('display') == 'none') && (jQuery(".Oursecondary").css('display')) == 'block')
{
	var prev_location = window.location.href.split('/')[4];
	var $current2 = $('#c_secondary > .active');
	$current2.removeClass();
	location.href= $("#c_catalogue").children().attr("href")+"?prevItem="+prev_location;
}
...
{
alert(getURLParameter(prevItem));
...
if((jQuery(".Oursecondary").css('display')) == 'block' && $current2.length == 0)
{
if(getURLParameter('prevItem'))
	{
		var prev = getURLParameter('prevItem');
		location.href=$("a[rel="+prev+"]").attr("href");
		$("a[rel="+prev+"]").parent().addClass("active");
	}
else
{
location.href=$("a[rel=tea-cakes]").attr("href");
